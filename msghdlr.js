...
const vcard = 'BEGIN:VCARD\n' 
            ...
            + 'FN:Admin akuuuu\n' 
            + 'ORG: Pengembang bot;\n'
            + 'TEL;type=CELL;type=VOICE;waid=6281237307921:+62 812-3730-7921\n'
            + 'END:VCARD' 
			...
			break
               ...
               case 'help': 
         	   ...
         	   case 'creator':
                  ...
                  client.sendMessage(from, 'Tuh nomer owner gua, lngsng save aja ngab dia masih jomblo:v',MessageType.text, { quoted: mek} )
					break    
				case 'leaderboard':
				case 'lb':
				bo = args[0]
           ...
           if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				if (!isQuotedSticker) return reply('𝗥𝗲𝗽𝗹𝘆/𝘁𝗮𝗴 𝘀𝘁𝗶𝗰𝗸𝗲𝗿 !')
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'udahh nohh '})
						fs.unlinkSync(ran)
					})
					await limitAdd(sender)
				break 
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				try {
						if (args.length < 1) return client.sendMessage(from, 'username nya mana', text, {quoted: mek})
           ...
           case 'demote':
				...
				case 'promote':
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `Selamat🥳 𝗮𝗻𝗱𝗮 𝗻𝗮𝗶𝗸 𝗺𝗲𝗻𝗷𝗮𝗱𝗶 𝗮𝗱𝗺𝗶𝗻 𝗴𝗿𝗼𝘂𝗽 (+_+) :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
